...
test('getOpenCases', async () => {
  const mock1 = mockPapi("get", { data: { events: "success" } });

  comp.$root.parameters.cases = { queries: [{ name: 'Open Cases', query: 'CUSTOM_QUERY' }] };
  comp.zone = 'Etc/Utc';

  const data1 = await comp.getOpenCases();

  expect(data1).toBe("success");
  expect(mock1).toHaveBeenCalledWith('events/', {
    params: {
      ...
      metricLimit: 10,
      zone: "Etc/Utc",
      range: expect.stringMatching(/^\d{4}\/\d{1,2}\/\d{1,2} \d{2}:\d{2}:\d{2} (A|P)M - \d{4}\/\d{1,2}\/\d{1,2} \d{2}:\d{2}:\d{2} (A|P)M$/),
      query: expect.stringContaining('CUSTOM_QUERY')
    }
  });

  resetPapi();
  const mock2 = mockPapi("get", { data: { events: "success" } });
  comp.$root.parameters.cases.queries = [];

  const data2 = await comp.getOpenCases();

  expect(data2).toBe("success");
  expect(mock2).toHaveBeenCalledWith('events/', {
    params: {
      eventLimit: 100,
      format: "2006/01/02 3:04:05 PM",
      metricLimit: 10,
      zone: "Etc/Utc",
      range: expect.stringMatching(/^\d{4}\/\d{1,2}\/\d{1,2} \d{1,2}:\d{2}:\d{2} (A|P)M - \d{4}\/\d{1,2}\/\d{1,2} \d{1,2}:\d{2}:\d{2} (A|P)M$/),
      query: '(NOT so_case.status:closed AND NOT so_case.category:template) AND _index:"*:so-case" AND so_kind:case'
    }
  });
});

test('createNewCase', async () => {
  const mock1 = mockPapi("post", { data: "success" });

  comp.newCaseTitle = 'Title';
  comp.newCaseDescription = 'Description';

  const data1 = await comp.createNewCase();

  expect(data1).toBe("success");
  expect(mock1).toHaveBeenCalledWith('case', {
    title: 'Title',
    description: 'Description',
  });

  resetPapi();
  const mock2 = mockPapi("post", { data: "success" });

  comp.newCaseDescription = '';

  const data2 = await comp.createNewCase();

  expect(data2).toBe("success");
  expect(mock2).toHaveBeenCalledWith('case', {
    title: 'Title',
