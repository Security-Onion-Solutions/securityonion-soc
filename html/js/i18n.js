...
const i18n = {
  translations: {
    "en-US": {
      ...
      eventsHealthDiskReadonly: 'Write-blocked (read-only) indices: {count}. Writes to these indices are rejected; Elasticsearch typically applies this block when disk usage exceeds the flood stage watermark.',
      ...
      eventsHealthReportDetails: 'Failure details',
      ...
      eventsHealthVerdictDiskThreshold: 'Nodes are above a disk usage watermark; free up or expand storage on the affected nodes.',
      eventsHealthVerdictNoValidShardCopy: 'No copy of these shards exists on any node in the cluster; their data is unavailable until a node holding a copy rejoins the grid.',
      eventsHealthVerdictSameShard: 'Replica shards cannot be placed on the same node as their primary; this is expected when the grid has a single search node.',
      eventsHealthVerdictThrottling: 'Shard recovery is temporarily throttled; this usually resolves on its own as recoveries complete.',
      eventsHealthVerdictUnexplained: 'Elasticsearch reports no blocking deciders for this group; allocation outcome: {value}.',
